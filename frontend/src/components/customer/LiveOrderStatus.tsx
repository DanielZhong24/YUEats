  ...
  return (
    <div className="space-y-6 mt-8">
      ...
      {activeOrders.map((order: any) => {
        ...
        return (
          <div key={order.id} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm">
              ...
              {STATUS_STEPS.map((step, idx) => (
                <div key={step} className="flex flex-col items-center">
                  <div className={`w-4 h-4 rounded-full border-4 ${
                    idx <= currentStep ? 'bg-green-500 border-green-200' : 'bg-white border-slate-200'
                  }`} />
            ...
            {order.status === 'READY_FOR_PICKUP' && (
              <div className="mt-6 p-4 bg-orange-50 rounded-xl border border-orange-100 flex items-start gap-3">
                ...
                <div>
                  <p className="font-bold text-orange-900">Ready at {order.deliveryAddress}!</p>
                  <p className="text-xs text-orange-700">Give your code to the driver to verify pickup.</p>
                </div>
              </div>
            )}
          </div>
