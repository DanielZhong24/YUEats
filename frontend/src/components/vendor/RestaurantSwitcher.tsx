...
export default function RestaurantSwitcher({ className }: { className?: string }) {
  const { restaurants, activeRestaurantId, setActiveRestaurantId, isLoading } = useVendorContext()

  if (isLoading) return <span className="text-sm text-slate-400 animate-pulse">Loading...</span>
